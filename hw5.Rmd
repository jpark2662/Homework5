...
```{r,  ECHO = TRUE}
setwd("c:/Users/Jim/Google~1/School/DoingDataScience/Week5")
library('reshape2')
df <- read.csv("yob2016.txt",stringsAsFactors = FALSE, sep = ";", header = FALSE)
library(data.table)
setnames(df, old = c('V1','V2','V3'), new = c('Name','Gender','AmountOfChildren'))
head(df)
```
...
```{r, ECHO = TRUE}
final <- merge( x = y2016[y2016$Gender == 'M',], y = y2015[y2015$Gender == 'M',], by = 'Name', all = FALSE)
head(final)
```
...
```{r, ECHO = TRUE}
girl <- merge( x = y2016[y2016$Gender == 'F',], y = y2015[y2015$Gender == 'F',], by = 'Name', all = FALSE)
...
ANSWER 3d. The top 10, most popular girl names from the combined y2015 and y2016 data sets are written to a csv fiel called 'itsagirl.csv'.
```{r, ECHO = TRUE}
